...
# local 
from net.percep_model3_skip5 import *
...
class Separator:
    def __init__(self, folder_name, image_file, 
                    ...
                    vgg_weight = 0.0, ssim_weight = 0.0,
                    # training info
        ...
        # --Type definition--
        self.LOSSES = namedtuple('LOSSES', ['loss', 'recon', 'gray', 'cross', 'excld','smooth', 'ceil', 'weak', 'tvex', 'vgg', 'ssim'])
    ...
    def _init_losses(self):
        ...
        # self.compute_ssim_loss = VGGLoss().to(self.device)
    ...
    def _compute_losses(self, step):
       ...
       # ---exclusion loss
        ...
        #                    * self.compute_vgg_loss(self.background_out, self.image_weak_b) ridam!!!!!!!!!!
        
        loss_out['vgg'] = loss_out['vgg'] + 1.0 * self.vgg_weight \
                           * self.compute_vgg_loss(self.background_out, self.image_weak_b)
        

        # loss_out['ssim'] = 1 - ms_ssim( self.background_out, self.image_weak_b, data_range=255, size_average=True )


        loss_out['ssim'] = loss_out['ssim']+ self.ssim_weight*(1 - ms_ssim( self.background_out, self.image_weak_b, data_range=255, size_average=True ))
...
if __name__ == "__main__":
    ...
    # mahsa = os.listdir("./Data/SamplesFromHuawei_CEIL")
    
    # # get_image_file = lambda n : '{}/{}'.format('./Huawei_I', n)

    # get_weak_file = lambda n : 'Data/SamplesFromHuawei_CEIL/{}/{}'.format(n, 't_output.png')


    # get_image_file = lambda n : 'Data/SamplesFromHuawei_CEIL/{}/{}'.format(n, 'input.png')



    mahsa = os.listdir("./Data/SamplestoTest")
    # mahsa = os.listdir("./Data/SamplestoTest")
    
    # get_image_file = lambda n : '{}/{}'.format('./Huawei_I', n)

    get_weak_file = lambda n : 'Data/SamplestoTest/{}/{}'.format(n, 'errnet.png')


    get_image_file = lambda n : 'Data/SamplestoTest/{}/{}'.format(n, 'm_input.png')
...
#    get_image_gt_r_file = lambda n :'{}/R/{}_r.jpg'.format(image_dir, n)
    ...
    enable_alphas = [1]
    ssim_weights = [0.1]
    # vgg_weights = [0.002, 0.025, 0.25, 0.4]
    vgg_weights = [0.20, 0.35]
    enable_alphaa = 1
    for vgg_weight in vgg_weights:
        for cur_ssim_weight in ssim_weights:
            for alpha_val in alpha_vals:
                ...
                for idx, name in enumerate(mahsa):
                    ...
                    model3 = Separator(
                            ...
                            image_weak_b_file = get_image_file(name),
                            # exp1 parameters -----------
                            image_size=im_size,
                            ...
                            enable_alpha = True if enable_alphaa == 1 else False, #True,
                            ...
                            excld_loss_weight = 0.12,
                            ceil_loss_weight  = 0.9, # 1 
                            smooth_loss_weight= 0.005, 
                            tvex_loss_weight  = 0.001, 
                            weak_weight = 0.15, #0.55,
                            vgg_weight = vgg_weight,
                            ssim_weight = cur_ssim_weight, #0.3,
                            # training parameters ------- 
                            num_iter=5300, learning_rate=0.0001,#0.0001 
                            ...
                            # outdir_name = 'Exp3_5_alphatest_withandwithoutSSIM_{}_alphaval_{}_alphaon_{}'.format(cur_ssim_weight, alpha_val, enable_alphaa)) #Test alpha on supervised

                            outdir_name = 'Exp3_5_FinalTest_Baghie__Vgg_{}_SSIM_{}_{}'.format(vgg_weight, cur_ssim_weight, alpha_val))

                            # outdir_name = 'Exp3_5_FinalTest1_SamplesFromHuawei_ERR_SSIM011_{}__{}'.format(cur_ssim_weight, alpha_val))
...
Exp3_5
                   ...
                   recon_loss_weight = 2.0, 
                    ...
                    # training parameters ------- 
                    num_iter=6000, learning_rate=0.0001,#0.0001 
                    show_every=500, plot_every=5000, 
                    outdir = './Output/',
                    
                    # outdir_name = 'ALL_SIR_Dialated_BjozGhablia{}'.format(alpha_val))
                    # outdir_name = 'kaggle_images2{}'.format(alpha_val))
                    outdir_name = 'Exp3_5{}'.format(""))
...
recon_loss_weight = 1.5, 
                        ...
                        # training parameters ------- 
                        num_iter=n_iter, learning_rate=0.0001,#0.0001 
                        show_every=500, plot_every=5000, 
                        outdir = './Output/',
                        
                        # outdir_name = 'ALL_SIR_Dialated_BjozGhablia{}'.format(alpha_val))
                        # outdir_name = 'kaggle_images2{}'.format(alpha_val))
                        outdir_name = 'Exp3_5_SamplesFromHuawei_ERR_testerideman_{}'.format(alpha_val))
                        #  outdir_name = 'H3{}'.format(alpha_val))
...
# settings fo rtesting weak-input
...
recon_loss_weight = 1.5, 
                            ...
                            outdir_name = 'Exp3_5_FinalTest_Baghie_SSIM01_{}'.format(alpha_val))
"""
