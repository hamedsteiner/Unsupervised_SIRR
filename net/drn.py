...
class up(nn.Module): 
    def __init__(self, in_ch, out_ch): 
        ...
        self.up = nn.Sequential(nn.ConvTranspose2d(in_ch, out_ch, 2, stride=2),
                                nn.LeakyReLU(), 
...
class drn_dip(nn.Module): 
    def __init__(self, model_name, classes=3, pretrained_model=None, pretrained=True): 
        super(drn_dip, self).__init__()
        ...
        self.up = nn.Sequential(up(512, 256), up(256, 128), up(128, 64))
        self.out = nn.Conv2d(64, classes, 1, bias=True) 
        self.sigmoid = nn.Sigmoid() 
#        self.conv = nn.Conv2d(model.out_dim, classes, kernel_size=1, bias=True)
#        self.up = nn.ConvTranspose2d(classes, classes, 16, stride=8, padding=4, 
#                                    output_padding=0, groups=classes, bias=False)
#        self.fill_up_weights()
#        self.up.requires_grad = False           
        
    def fill_up_weights(self):   
        # init weights 
        m = self.conv
        num_weight = m.kernel_size[0]  * m.kernel_size[1] * m.out_channels
        m.weight.data.normal_(0, math.sqrt(2. / num_weight))
        m.bias.data.zero_()

        # init weights for upsampling block 
