'''
Date: March, 2021 
Author: Hamed RahmaniKhezri, Suhong Kim

Using 448 -> 
            add two layers before and after latent
'''
...
# class Down448(nn.Module):
    # def __init__(self, in_ch, out_ch):
        # super(Down448, self).__init__()
        # self.mpconv = nn.Sequential(
            # SpectralNorm(nn.Conv2d(in_ch, in_ch, 3, padding=1, stride=2)),
            # nn.LeakyReLU(),
            # double_conv(in_ch, out_ch)
...
# class Up448(nn.Module):
    # def __init__(self, in_ch, out_ch, bilinear=False):
        # # super(Up448, self).__init__()
# 
        # if bilinear:# resize-conv
            # self.up = nn.Sequential(
                # nn.Upsample(scale_factor=2, mode='bicubic'),
...
class PercepNet(nn.Module): 
    def __init__(self, n_channels, n_classes, image_t,  pretrained='places365',enable_attn=False): 
        ...
        """
        changed these parts and a bit of channels
        """


        self.down448 = down(256, 128)
        self.up448 = up(128, 128)
