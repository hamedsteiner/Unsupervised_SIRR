...
class PercepNet(nn.Module): 
    ...
    def forward(self, x):
        ...
        # print("in_down3 ",in_down3.shape)
        # print("self.feats[1].detach() ",self.feats[1].detach().shape)
        # print("In1 ",in1[:, 32:, :, :].shape)
        # exit()
        ...
        up2 = self.Up2(torch.cat([up1, in2], dim=1)) #28

        # up3 = self.Up3(up2) #56
        up3 = self.Up3(torch.cat([up2, in1[:, 32:, :, :]], dim=1)) #56           #REsnet
