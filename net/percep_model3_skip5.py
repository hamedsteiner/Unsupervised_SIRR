...
class PercepNet(nn.Module): 
    ...
    def forward(self, x):
        ...
        up1 = self.Up1(torch.cat([y_enc, in3], dim=1)) #14   ## CAN BE CHANGE LATER
        up2 = self.Up2(up1) #28

        # up3 = self.Up3(up2) #56
        up3 = self.Up3(torch.cat([up2, in_down3], dim=1)) #56

        up4 = self.Up4(torch.cat([up3, in_down2], dim=1)) #112

        # up5 = self.Up5(up4) #224
        up5 = self.Up5(torch.cat([up4, in_down1], dim=1)) #224
