...
class PercepNet(nn.Module): 
    def __init__(self, n_channels, n_classes, image_t,  pretrained='places365',enable_attn=False): 
        ...
        # self.param_decoder = [p for p in self.decoder.parameters()] + \
                            # [p for p in self.output.parameters()]
# 
        # self.param_encoder = [p for p in self.input.parameters()] + \
                            # [p for p in self.down1.parameters()]  + \
                            # [p for p in self.down2.parameters()]  + \
                            # [p for p in self.down3.parameters()]
                            # 
        # self.NUM_dec = sum(np.prod(list(p.size())) for p in self.param_decoder)
        # self.NUM_enc = sum(np.prod(list(p.size())) for p in self.param_encoder)
# 
        # print("PARAAAAAAM  DECODER : ",self.NUM_dec)
        # print("PARAAAAAAM  ENCODER : ",self.NUM_enc)
